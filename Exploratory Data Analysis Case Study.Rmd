...
# Create a dataframe to save the missing values
...
Visualization indicates a small proportion of missing value (less than 0.2%). This indicates a random distribution of the missing values. MCAR test was performed to validate the randomness of variables.
...
Missing Product_ID and Customer_ID will be dropped. The missing outcome variable Purchase will be replaced by the median due to the left-skewed distribution. For other categorical variable, they will be replaced by the modes.
...
categorical_columns <- c("Gender", "Age", "Occupation", "City_Category", "Stay_In_Current_City_Years", "Marital_Status", "Product_Category_1", "Product_Category_2", "Product_Category_3")
...
z_scores[z_scores > 3] # Check Outliers
...
summarize_factor(BlackFriday_cleaned)
```
